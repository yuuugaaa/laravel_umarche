                    ...
                    <section class="text-gray-600 body-font relative">
                        <div class="container px-5 py-4 mx-auto">
                            ...
                            <form id="delete_{{ $product->id }}" method="post" action="{{ route('owner.products.destroy', ['product' => $product->id]) }}">
                                @csrf
                                @method('delete')
                                <div class="p-2 pt-12 w-full flex justify-around mt-12 border-t-2 border-gray-200">
                                    <a href="#" data-id="{{ $product->id }}" onclick="deletePost(this)" class="text-white bg-red-500 border-0 py-2 px-12 focus:outline-none hover:bg-red-600 rounded text-lg">削除</a>
                                </div>
                            </form>
                        </div>
                    </section>
