                        ...
                        <div class="flex flex-wrap -m-4">
                            @foreach ($shops as $shop)
                                ...
                                <a href="{{ route('owner.shops.edit', ['shop' => $shop->id]) }}" class="block h-full border-2 border-gray-200 border-opacity-60 rounded-lg overflow-hidden">
                                    @if ($shop->filename)
                                        <img class="lg:h-48 md:h-36 w-full object-cover object-center" src="{{ asset('storage/shops'. $shop->filename) }}" alt="shop">
                                    @else
                                        <img class="lg:h-48 md:h-36 w-full object-cover object-center" src="{{ asset('images/no_image.jpg') }}" alt="no image">
                                    @endif
                                    <div class="p-4">
                                        @if ($shop->is_selling)
